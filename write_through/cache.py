...
class sim_cache:
    ...
    def read(self,target_file):
        ...
        #检查缓存中是否已缓存文件
        info = self._read_cache(target_file)
        #向server确认version是否一致
        server_version = self._target_server.get_version(target_file)
        
        #选择从cache/server中读取文件
        if info is not None and server_version == info[2]:
            return info[1]
            
        #缓存从server中读取的文件
        info = self._target_server.read(target_file)
        self._write_cache(target_file,info[1],info[2])
        return info[1]
        ######## End ########
